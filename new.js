  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      $("tb_materai tr.gen_datam").each(function (k, v) {
        ...
        $(this)
          .find("td.e-materai")
          .attr("id", "gen_boxm_" + idx + "_m");
        $(this)
          .find("td.halaman-materai")
          .attr("id", "gen_boxm_" + idx + "_p");
        $(this)
          .find("td.x-materai")
          .attr("id", "gen_boxm_" + idx + "_x");
        $(this)
          .find("td.y-materai")
          .attr("id", "gen_boxm_" + idx + "_y");

        $(this)
          .find("td > a")
          .attr({
            id: "gen_boxm_" + idx + "_delete",
            target: idx,
          });
      });
...
function resetData() {
  ...
  $(".gen_data").remove();
  $(".stretch").remove();
  $(".card-signers").css("display", "nonde");
...
window.addEventListener("load", function () {
  ...
  $(function () {
      ...
      $("#users tbody").append(
        "<tr id='gen_data_" + i + "' data-flag='" + i + "' class='gen_data'>" + 
          "<td>" + 
            ...
            "<div class='custom-control custom-checkbox' style='zoom: 150%;margin-top:10px'><input type='checkbox' class='custom-control-input' id='gen_box_" + i + "_hide'> <label class='custom-control-label' for='gen_box_" + i + "_hide' style='font-weight: bold; font-size: 11px; padding-top: 3px;'>Sembunyikan</label></div> </td>" +
          ...
          "<td align='center'><a id='gen_box_" + i + "_delete' target='" + i + "' class='btn btn-sm btn-danger' onclick='window.deleteItem(this)'>Delete</a></td>" +
        "</tr>"
      );
      i++;
    ...
    $(document).on("click", "#materai", (e) => {
      ...
      i = $("#tb_materai tr.gen_datam").length + 1;
      console.log("posX :", posX, "posY :", posY);
      //append a new div and increment the class and turn it into jquery selector
      $(".canvas-here").append(
        '<div data-flagm="' + iMaterai + '" target="gen_boxm_' + iMaterai + '" class="stretch gen_boxm_' + iMaterai + '" page="' + pageNow + '">' + 
        '<h2 class="watermark">' + iMaterai + "</h2></div>"
      );
      gen_box = $(".gen_boxm_" + iMaterai);
      ...
      $("#tb_materai tbody").append(
        ...
        <tr id='gen_datam_`+ iMaterai + `' data-flagm='`+ iMaterai +`' class='gen_datam'>
          ...
          <td><a id='gen_boxm_` + iMaterai + `_delete' target='` + iMaterai + `' class='btn btn-sm btn-danger' onclick='window.deleteItemMaterai(this)'>Delete</a></td>
        </tr>
        `
      );
      iMaterai++;
...
$("#generate-json-materai").on("click", function (e) {
  ...
  for (var q = 1; q <= $(".gen_datam").length; q++) {
    ...
    //console.log(object["document_type"] = $("#document_type :selected").val());
    object["detail"] = [];
    var detail = {
...
$("#generate-json-materai").on("click", function (e) {
  ...
  for (var q = 1; q <= $(".gen_datam").length; q++) {
    ...
    var details = {
      ...
      for (let z = json_post.length - 1; z >= 0; z--) {
